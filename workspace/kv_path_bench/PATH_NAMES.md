...
两项单段实验复用中转路径的读取与地址布局，每轮清空 L2，单独采样。校验默认关闭；启用 `--validate` 时，Mooncake 在计时外直接检查 Host 内容，MemFabric 在计时外用已有的本地 GH2L 读出 L2 并逐字节校验，此验证拷贝不计入单段耗时。保留原五档容量和两种布局；对单段而言连续/分散指后续 L1 映射，两者 L2 地址组织相同，不能解释成 L2 碎片对照。时间单位不变；统计改为整请求实际耗时，见套件 README。状态/错误标记不属于路径名。
...
E/F/D 都在 `fabric_direct_bench`。它们共用 BM SDMA、同一源数据、相同 L1 地址、整请求提交。E/F 的 L2 使用与 A/C 相同的分离 KV/RoPE page-first 布局、请求页数加一个保留槽；F 不增加接收暂存。E 在计时前填充 L2，F 将两段传输及中间等待计入时间。每条路径以完整请求预热、采样，仅启用 `--validate` 时每轮在计时外校验。

A/E、C/F 用于同路径的软件实现对照；F/D 用于同软件栈的中转/直达对照。仍存在 ADXL 与 BM 内存分配器、Store 管理开销和源对象连续性差异，不将所有差异归因于单一算子。整请求版本尚需远端校验，脚本先一页冒烟再扩大，性能数据校验需显式启用 `--validate`。
