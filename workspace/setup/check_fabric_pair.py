...
Uses the same seven-argument Store API as check_fabric_local.py.
Client contributes no segment. Writer exits before a fresh reader verifies data.
PASS proves remote Store correctness, not the physical HCCS route or bandwidth.
"""
import argparse
import hashlib
import ipaddress
...
from check_fabric_local import mark, stop
...
def worker(args):
    ...
    print("Mooncake", importlib.metadata.version("mooncake-transfer-engine-npu"), flush=True)
    mark(directory, "DEVICE")
    torch.npu.set_device(args.device)
    mark(directory, "SETUP")
    store = MooncakeDistributedStore()
    segment = (1 << 30) if args.mode == "target" else 0
    print("protocol=ascend fabric=1 global_segment_size=", segment, flush=True)
    rc = store.setup(args.local_ip, "P2PHANDSHAKE", segment, 1 << 30,
                     "ascend", "", f"{args.target_ip}:{args.port}")
    if rc != 0:
...
def main():
    ...
    try:
        ...
        if args.device < 0 or args.timeout < 1 or not 1024 <= args.port <= 65535:
    ...
    try:
        ...
        print("F2:CHECK_IPS")
        return 1
    directory = Path(tempfile.mkdtemp(prefix="mooncake-fabric-pair-"))
    print("Logs:", directory, flush=True)
    print("NPU", args.device, "; 1 GiB buffer; target alone contributes 1 GiB Store.", flush=True)
    env = os.environ.copy()
    ...
    try:
        if args.mode == "target":
            ...
            print("F2:READY — leave this terminal running; Ctrl+C after client finishes.", flush=True)
        ...
        launch("read")
        print("F2:REMOTE_OK")
        print("Fresh-process remote read verified. Fabric was requested; physical HCCS route needs log verification.")
        return 0
