...
本文件适用于本仓库中由 Agent 协助开展的 Ascend 远端实验。

## 文档分工

- `ascend-setup.md`：环境、容器和依赖安装。
- `fabric-note.md`：本地保存的测试事实、命令、日志和结果；不提交 Git。
- `research-status.md`：当前研究目标、已确认结论、方向和边界。
- `kv-path-plan.md`：当前实验设计和交接任务。
- 旧的正式实验方案只作历史参考，不作为当前执行入口。

不要把测试结果写进环境部署文档，也不要把临时判断写进实验计划。研究方向变化时更新 `research-status.md`，实验设计变化时更新 `kv-path-plan.md`。
...
- Ascend 构建运行 TCP probe 时，脚本需要设置 `MC_FORCE_TCP=1`，避免引擎初始化时自动安装 Ascend transport。
...
- 28 机器曾在测试期间 down，原因未确认。未确认原因前不要重复大内存/NPU 测试，也不要把该次结果当作性能数据。
...
- 先对齐现有 HiCache 的对象管理：一个逻辑 page 对应一个 Store key，一次可以批量读取多个对象。
- 不自行改变 page 对象粒度、请求数据或目标地址后，再把差异归因于传输路径。
- L1、L2 使用当前 Ascend MLA 的实际布局；L3 的对象组织和片段顺序在任务中明确写出。新增布局、对象聚合、缓存策略或流水优化都要单独标记为实验变量。
- 三条路径使用相同逻辑 KV、相同 page 映射和相同批量划分；必要的转换、拷贝、同步和暂存内存计入对应路径。
...
收到结果后，先更新本地 `fabric-note.md` 的事实记录；只有研究结论或实验设计改变时，才分别更新 `research-status.md` 或 `kv-path-plan.md`。
